...
/* localStorage shims matching the artifact-only window.storage API shape,
   so every call site below works unchanged outside the chat sandbox.
   IMPORTANT: localStorage is per-browser, not shared across users/devices.
   The `shared: true` flag from the original API has no real equivalent
   here — SHARED_IMAGES_KEY (admin-set exercise images) will NOT actually
   be shared across different users' browsers with this shim. For real
   cross-user sharing, move SHARED_IMAGES_KEY reads/writes to a Supabase
   table (e.g. a public-read, admin-write `exercise_images` table) instead. */
function lsGetAsync(key) {
  return new Promise((resolve) => {
    try { const v = localStorage.getItem(key); resolve(v ? { value: v } : null); }
    catch (e) { resolve(null); }
  });
}
function lsSetAsync(key, value) {
  return new Promise((resolve) => {
    try { localStorage.setItem(key, value); } catch (e) {}
    resolve();
  });
}
function lsDeleteAsync(key) {
  return new Promise((resolve) => {
...
async function loadSession() {
  try {
    const res = await lsGetAsync(SESSION_KEY);
...
async function saveSession(session) {
  ...
  try {
    if (session) await lsSetAsync(SESSION_KEY, JSON.stringify(session));
    else await lsDeleteAsync(SESSION_KEY);
  } catch (e) { /* ignore */ }
}
...
async function loadSharedExerciseImages() {
  try {
    const res = await lsGetAsync(SHARED_IMAGES_KEY);
    if (res?.value) return JSON.parse(res.value);
  } catch (e) { /* none set yet */ }
  return {};
}
async function saveSharedExerciseImages(images) {
  try { await lsSetAsync(SHARED_IMAGES_KEY, JSON.stringify(images)); } catch (e) { /* ignore */ }
}
...
async function loadLocalState() {
  try {
    const res = await lsGetAsync(LOCAL_KEY);
    if (res && res.value) return JSON.parse(res.value);
  } catch (e) { /* first run */ }
  return {};
}
async function saveLocalState(partial) {
  try { await lsSetAsync(LOCAL_KEY, JSON.stringify(partial)); } catch (e) { /* ignore */ }
}
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const raw = await lsGetAsync(THEME_PREFS_KEY);
        if (raw?.value) {
  ...
  useEffect(() => {
    lsSetAsync(THEME_PREFS_KEY, JSON.stringify({ appearance, themeName })).catch(() => {});
  ...
  const enterGuestMode = useCallback(async () => {
    ...
    let guestState = null;
    try {
      const raw = await lsGetAsync(GUEST_KEY);
  ...
  const exitGuestMode = useCallback(async () => {
    try { await lsDeleteAsync(GUEST_KEY); } catch (e) { /* ignore */ }
    setIsGuest(false);
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      if (session) {
        ...
        lsSetAsync(GUEST_KEY, JSON.stringify(state)).catch(() => {});
      }
    }, 500);
